...
def get_release_history_data(snap_name):
    release_history_cache_key = get_release_history_key(snap_name)
    cached_data = redis_cache.get(release_history_cache_key)
    if cached_data:
        return flask.jsonify({"success": True, "data": cached_data})
    ...
    redis_cache.set(release_history_cache_key, context, ttl=300)
